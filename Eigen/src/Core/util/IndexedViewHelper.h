...
struct IndexedViewCompatibleType<T,XprSize,typename std::enable_if_t<internal::is_integral<T>::value>> {
...
template<typename T, int XprSize>
struct IndexedViewCompatibleType<T, XprSize, typename std::enable_if_t<Symbolic::is_symbolic<T>::value>> {
  typedef SingleRange type;
};
